...
class DataIngestionPipeline {
  ...
  private async scrapeContent(): Promise<ScrapedContent[]> {
    if (this.options.skipScraping) {
      ...
      if (existingFiles.length > 0) {
        console.log(`📂 Loaded ${existingFiles.length} previously scraped files`);
        return existingFiles;
      }
      throw new Error('No previously scraped content found and scraping is disabled');
    }

    console.log('🕸️  Step 1: Scraping website content...');

    // Extract sitemap URLs or use base URL
    const urls = await this.providers.scraper.extractSitemapUrls(this.config.scraper.baseUrl);
    ...
    for (let i = 0; i < Math.min(urls.length, this.options.maxPages); i++) {
      const url = urls[i];
      console.log(`  [${i + 1}/${Math.min(urls.length, this.options.maxPages)}] Scraping: ${url}`);
   ...
   * Step 2: Validate scraped content
   */
  private async validateContent(content: ScrapedContent[]): Promise<ScrapedContent[]> {
    ...
    for (const item of content) {
      ...
      if (!item.content || item.content.trim().length < 100) {
        console.log(`  ⚠️  Skipping ${item.url}: Content too short (${item.content?.length || 0} chars)`);
        skippedCount++;
        continue;
      }
      ...
      if (this.options.validateUrls) {
        try {
          const validation = await this.validationService.validateUrl(item.url, {
            checkUrlStatus: false,
            trustKnowledgeBase: true,
          });

          if (!validation.valid && this.options.verbose) {
            console.log(`  ⚠️  URL validation warnings for ${item.url}:`, validation.warnings);
          }
        } catch (error) {
  ...
  private async saveRawContent(content: ScrapedContent[]): Promise<void> {
    ...
    const outputFile = path.join(this.options.outputDir, `raw-content-${timestamp}.json`);
    ...
    if (this.options.verbose) {
      const fullContentFile = path.join(this.options.outputDir, `full-content-${timestamp}.json`);
   ...
   * Step 4: Chunk content
   */
  private async chunkContent(content: ScrapedContent[]): Promise<VectorDocument[]> {
    ...
    for (const item of content) {
      ...
      if (this.options.verbose) {
        ...
        console.log(`    → Avg size: ${result.statistics.averageChunkSize} chars`);
   ...
   * Step 5: Generate embeddings
   */
  private async generateEmbeddings(documents: VectorDocument[]): Promise<VectorDocument[]> {
    if (this.options.skipEmbedding) {
      console.log('⏭️ Skipping embedding generation (skipEmbedding flag is set)');
    ...
    try {
      ...
      const embeddingResult = await this.providers.embeddings.generateEmbeddings(texts);
      ...
      console.log(`  📊 Embedding dimension: ${embeddingResult.embeddings[0]?.length || 'unknown'}`);
  ...
  private async generateReport(
    ...
    const reportFile = path.join(this.options.outputDir, `ingestion-report-${timestamp.replace(/[:.]/g, '-')}.json`);
    ...
    console.log(`With embeddings:     ${report.statistics.documentsWithEmbeddings}`);
  ...
  private loadExistingScrapedContent(): ScrapedContent[] {
    const files = fs.readdirSync(this.options.outputDir)
      .filter(file => file.startsWith('full-content-') && file.endsWith('.json'))
      .sort()
...
export { DataIngestionPipeline, IngestionOptions };
```

Now I need to add this to package.json scripts. Let me check the current package.json.
