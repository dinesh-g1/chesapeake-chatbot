...
chesapeake-chatbot/components/ChatInterface.tsx
```
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  useEffect(() => {
    if (!currentSessionId) {
      const storedSessionId = localStorage.getItem('chesapeake_chat_session_id');
      if (storedSessionId) {
  ...
  useEffect(() => {
    if (currentSessionId) {
      try {
        const savedMessages = localStorage.getItem(`chesapeake_chat_messages_${currentSessionId}`);
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-semibold text-gray-700 mb-2">Welcome to Chesapeake Chat Assistant</h3>
              <p className="text-gray-500 max-w-md mx-auto">
                I can help you with information about Chesapeake City services, departments, events, and more.
                Try asking about permits, utilities, city services, or local events.
              </p>
            </div>
          ) : (
            <>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium opacity-80">
                        {message.role === 'user' ? 'You' : 'Chesapeake Assistant'}
                      </span>
                    ...
                    {message.role === 'assistant' && message.suggestedFollowUps && message.suggestedFollowUps.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-300 border-opacity-30">
                        <p className="text-xs font-medium mb-2 opacity-80">Suggested follow-ups:</p>
                    ...
                    {message.metadata && (
                      <div className="mt-2 pt-2 border-t border-gray-300 border-opacity-20">
                        <p className="text-xs opacity-60">
                          {message.metadata.responseTime && `Response: ${message.metadata.responseTime}ms`}
              ...
              {isTyping && streamingContent && (
                <div className="flex justify-start">
                  <div className="max-w-[80%] rounded-2xl rounded-bl-none bg-gray-100 text-gray-800 px-5 py-3 border border-gray-200">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs font-medium opacity-80">Chesapeake Assistant</span>
        ...
        <div className="border-t border-gray-200 p-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex space-x-4">
              ...
              <div className="flex flex-col justify-end space-y-2">
                <button
                  ...
                  className={`px-6 py-3 rounded-lg font-medium transition-colors ${!input.trim() || isLoading
            ...
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => handleFollowUpClick('What are the trash pickup schedules?')}
              ...
              <button
                type="button"
                onClick={() => handleFollowUpClick('How do I apply for a building permit?')}
                className="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-full transition-colors"
                disabled={isLoading}
              >
                Building permits
              ...
              <button
                type="button"
                onClick={() => handleFollowUpClick('What city services are available online?')}
                className="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-full transition-colors"
                disabled={isLoading}
              >
                Online services
              ...
              <button
                type="button"
                onClick={() => handleFollowUpClick('How do I contact the police department?')}
                className="text-sm bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-1.5 rounded-full transition-colors"
                disabled={isLoading}
              >
                Police contact
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
              ...
              <div className="flex items-center space-x-4">
                <span>Streaming: {streamingEnabled ? 'Enabled' : 'Disabled'}</span>
